...
public class AppInstanceBasicsTestCase : ICliTestCase
{
    public async Task ExecuteAsync(SeqConnection connection, ILogger logger, CliCommandRunner runner)
    {
        ...
        exit = runner.Exec("appinstance create", $"-t {Guid.NewGuid():N} --app {app.Id} --stream-signal=\"{streamSignal}\" -p To=example@example.com -p From=example@example.com -p Host=localhost");
        Assert.Equal(0, exit);
    }
}
