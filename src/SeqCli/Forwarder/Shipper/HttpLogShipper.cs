...
sealed class HttpLogShipper : LogShipper
{
    ...
    void MakePayload(LogBufferEntry[] entries, bool oneOnly, out Stream utf8Payload, out ulong lastIncluded)
    {
        ...
        content.Flush();
        var contentRemainingBytes = (int) _outputConfig.PayloadLimitBytes - 13; // Includes closing delimiters
        ...
        foreach (var logBufferEntry in entries)
        {
            ...
            {
                Log.Information("Oversize event will be skipped, {Payload}", Encoding.UTF8.GetString(logBufferEntry.Value));
